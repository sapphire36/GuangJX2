...
public class HttpsUtil extends DefaultHttpClient {
	...
	public void initSSLConfigForTwoWay() throws Exception {
		...
		String selfcertpath = demo_base_Path + Constants.SELFCERTPATH;
		String trustcapath = demo_base_Path + Constants.TRUSTCAPATH;

		KeyStore selfCert = KeyStore.getInstance("pkcs12");
		selfCert.load(new FileInputStream(selfcertpath),
				Constants.SELFCERTPWD.toCharArray());
		KeyManagerFactory kmf = KeyManagerFactory.getInstance("sunx509");
		kmf.init(selfCert, Constants.SELFCERTPWD.toCharArray());

		// 2 Import the CA certificate of the server,
		KeyStore caCert = KeyStore.getInstance("jks");
		caCert.load(new FileInputStream(trustcapath), Constants.TRUSTCAPWD.toCharArray());
